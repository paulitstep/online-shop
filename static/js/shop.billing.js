...
if (paymentForm.length > 1) {
    alert('На каждую страницу - одна платежная форма')
...
//   var loadTime = 1500
//   var errorHtml = '<i class="fa fa-warning"></i> Произошла ошибка!'
//   var errorClasses = 'btn btn-danger disabled my-3'
//   var loadingHtml = '<i class="fa fa-spin fa-spinner"></i> Загрузка...'
...
function reidrectToNext(NextPath, timeoffset) {
    if (NextPath){
    setTimeout(function(){
...
function stripeTokenHandler(nextUrl, token) {
  ...
  $.ajax({
    ...
    success: function(data){
        var successMsg = 'Ваша банковская карточка добавлена!'
        card.clear()
        if (nextUrl){
            successMsg = successMsg + '<br/><br/><i class="fa fa-spin fa-spinner"></i> Переадресация...'
        }
        if ($.alert){
            $.alert(successMsg)
        } else {
        ...
        reidrectToNext(nextUrl, 1500)
    },
    error: function(error){
        $.alert({title: 'Произошла ошибка!', content: 'Попробуйте еще раз.'})
        btnLoad.html(btnLoadDefaultHtml)
        btnLoad.attr('class', btnLoadDefaultClasses)
    }
  })
